...
(function($) {
  ...
  $.cleditor = {
    ...
    buttons: {
      init: [
		...
		['image', __("Insert image"), 'insertimage', 'imageurl'],
  ...
  function buttonClick(e) {
    ...
    // Check for rich text mode
      ...
      if (popupName) {
        ...
        if (popupName == "imageurl") {

          // Wire up the submit button click event handler
          $popup.children(":button.send")
            .unbind(CLICK)
            .bind(CLICK, function() {
			...
			$popup.children(":button.file")
            .unbind(CLICK)
            .bind(CLICK, function() {
				var $text = $popup.find(":text");
				$(this).upload( {
					autoSubmit: false,
					action: './plug.php?r=cleditor',
					onSelect: function() {
					...
					onComplete: function(response) { 
						...
						if(response.error && response.error.length > 0)
							...
							$text.val((response.file && response.file.length > 0) ? response.file : '');
					}
				});
            });
        }
  ...
  function createPopup(popupName, options, popupTypeClass, popupContent, popupHover) {
    ...
    // URL
      ...
      $popup.html(__('Enter image SRC:') + '<br><input type=text value="http://" size=35><br><input type=button class="file" value="Upload" /> <input type=button class="view" value="View" /> <input class="send" type=button value="' + __('Submit') + '">');
      popupTypeClass = PROMPT_CLASS;
    }	
